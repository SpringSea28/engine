...
IMPLEMENT_WRAPPERTYPEINFO(ui, BarcodeParagraphBuilder);

void BarcodeParagraphBuilder::Create(Dart_Handle wrapper,
  ...
  auto res = fml::MakeRefCounted<BarcodeParagraphBuilder>(
...
void decodeStrut2(Dart_Handle strut_data,
...
BarcodeParagraphBuilder::BarcodeParagraphBuilder(
  ...
  if (mask & kPSStrutStyleMask) {
    decodeStrut2(strutData, strutFontFamilies, style);
  }
...
BarcodeParagraphBuilder::~BarcodeParagraphBuilder() = default;

void decodeTextShadows2(
...
void decodeFontFeatures2(Dart_Handle font_features_data,
...
void decodeFontVariations2(Dart_Handle font_variations_data,
...
void BarcodeParagraphBuilder::pushStyle(const tonic::Int32List& encoded,
...
//  txt::TextStyle style = m_paragraph_builder_->PeekStyle();
  ...
  if (mask & kTSTextShadowsMask) {
    decodeTextShadows2(shadows_data, style.text_shadows);
  }
  ...
  if (mask & kTSFontFeaturesMask) {
    decodeFontFeatures2(font_features_data, style.font_features);
  }

  if (mask & kTSFontVariationsMask) {
    decodeFontVariations2(font_variations_data, style.font_variations);
  }
...
void BarcodeParagraphBuilder::pop() {

}

Dart_Handle BarcodeParagraphBuilder::addText(const std::u16string& text) {

  return Dart_Null();
}

void BarcodeParagraphBuilder::addPlaceholder(double width,
                                      double height,
                                      unsigned alignment,
...
std::u16string utf8ToUtf16(const char* utf8Str) {
  FML_LOG(ERROR) << "utf8ToUtf16 " << utf8Str;
  UErrorCode status = U_ZERO_ERROR;
  int32_t destLength = 0;
  UChar* dest = u_strFromUTF8(nullptr, 0, &destLength, utf8Str, -1, &status);
  FML_LOG(ERROR) << "utf8ToUtf16 destLength: " << destLength;
  FML_LOG(ERROR) << "utf8ToUtf16 status: " << status;
  if (U_FAILURE(status)) {
    // 处理错误
    return std::u16string();
  }
//  if (!(status == U_BUFFER_OVERFLOW_ERROR && destLength > 0)) {
//    // 处理错误
//    return std::u16string();
//  }

  std::u16string utf16Str(destLength, u'\0');
  u_strFromUTF8(utf16Str.data(), destLength + 1, &destLength, utf8Str, -1, &status);
  FML_LOG(ERROR) << "utf8ToUtf16 status2: " << status;
  if (U_FAILURE(status)) {
    // 处理错误
    return std::u16string();
  }
  FML_LOG(ERROR) << "utf8ToUtf16: " << utf16Str.data();
  return utf16Str;

//  FML_LOG(ERROR) << "utf8ToUtf16 " << utf8Str;
//  icu::UnicodeString unicodeStr(utf8Str, -1, US_INV);
//  std::u16string utf16Str = unicodeStr.getBuffer();
//  FML_LOG(ERROR) << "utf8ToUtf16: " << utf16Str.data();
  return utf16Str;
...
//sojet modify
std::unique_ptr<txt::Paragraph> BarcodeParagraphBuilder::build() {
  FontCollection& font_collection = UIDartState::Current()
                                        ->platform_configuration()
                                        ->client()
                                        ->GetFontCollection();
  ...
  paragraph_builder->PushStyle(textStyle);
...
//  const UChar* text_ptr = reinterpret_cast<const UChar*>(fInput);
//  UErrorCode error_code = U_ZERO_ERROR;
//  u_strToUTF8(nullptr, 0, nullptr, text_ptr, inputLen+1, &error_code);
//  if (error_code != U_BUFFER_OVERFLOW_ERROR) {
//    FML_LOG(ERROR) << "string is not well-formed UTF-16" ;
//  }
  FML_LOG(ERROR) << "utf8ToUtf16 fInput" << fInput;
...
void BarcodeParagraphBuilder::setText(const char* text){
  this->text = text;
}
